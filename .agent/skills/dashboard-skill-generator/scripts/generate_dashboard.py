"""Dashboard Skill Generator — 三層架構：JSON → DSL → Renderer → HTML

CLI 用法：
    py scripts/generate_dashboard.py --input data.json
    py scripts/generate_dashboard.py --input data.json --output out.html --title "營收分析"
...
def render_table(widget: dict, data: dict) -> tuple[str, str]:
    ...
    # 表頭
    th_html = "".join(f'<th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">{col}</th>' for col in columns)

    # 表身
    rows_html = []
    for i, row in enumerate(source):
        bg = "bg-slate-50/50" if i % 2 == 0 else ""
        cells = "".join(f'<td class="px-4 py-3 text-sm text-slate-700">{row.get(col, "")}</td>' for col in columns)
...
def render_all_widgets(dsl: dict, data: dict) -> tuple[str, str]:
    ...
    # 排序：kpi_group 先，chart 中間，table 最後
...
def main():
    parser = argparse.ArgumentParser(description="Dashboard Skill Generator — JSON → DSL → HTML")
    parser.add_argument("--input", required=True, help="JSON 資料檔案路徑")
    parser.add_argument("--output", default=None, help="HTML 輸出路徑（預設 data/dashboard/{type}/）")
    parser.add_argument("--dsl", default=None, help="自訂 DSL JSON 檔案路徑")
    parser.add_argument("--mode", default="auto", choices=["auto", "dsl"], help="模式：auto（AI 產 DSL）或 dsl")
    parser.add_argument("--title", default=None, help="儀表板標題")
    parser.add_argument("--theme", default="light", choices=["light", "dark"], help="主題")
    ...
    if not input_path.exists():
        print(f"[ERROR] 找不到 JSON 檔案：{args.input}")
        sys.exit(1)
    ...
    if args.dsl:
        ...
        if not dsl_path.exists():
            print(f"[ERROR] 找不到 DSL 檔案：{args.dsl}")
            sys.exit(1)
        with open(dsl_path, "r", encoding="utf-8") as f:
            custom_dsl = json.load(f)
    ...
    if args.output:
        ...
        shutil.move(result["html_path"], str(final_path))
        result["html_path"] = str(final_path)

    print(f"[OK] 儀表板已產生：{result['html_path']}")
    print(f"     widgets={result['metadata']['widgets']}, "
          f"charts={result['metadata']['charts']}, "
          f"tables={result['metadata']['tables']}, "
          f"kpi={result['metadata']['kpi_cards']}")
